...
★ [JSON 데이터 출력 규칙 - 최우선 순위]:
1. 당신의 답변은 무조건 다음 순서를 지키세요: 
   - [STEP 1] 사용자의 요청을 수락하거나 대안을 제시하는 짧은 확인 멘트.
   - [STEP 2] 만약 확정이 필요하다면, 그 즉시 줄바꿈 후 [JSON_DATA]...[/JSON_DATA]를 출력.
2. [JSON_DATA] 블록 내부에는 절대로 다른 잡담이나 텍스트를 포함하지 마세요.
3. 여러 개의 일정을 생성할 경우, 반드시 [JSON_DATA]{...}[/JSON_DATA]를 각각 독립적으로 줄을 바꿔서 출력하세요.
4. 파이썬이 이 데이터를 읽어야 하므로, [JSON_DATA] 바로 앞에 다른 텍스트를 길게 쓰지 마세요.
