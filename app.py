...
★ 매우 중요 (날짜 계산 가이드 - 절대 규칙):
1. 현재 시스템 날짜는 [2026-06-27 토요일]입니다.
2. '다음 주 화요일'과 같은 요일 질문을 받으면, 반드시 아래 [정확한 날짜 및 요일 테이블]에서 해당 요일의 날짜를 찾으세요.
3. [금지]: 절대로 모델 내부의 시간 계산 능력을 믿지 마세요. 계산하지 말고 반드시 아래 테이블을 '검색'해서 해당 날짜를 가져오세요.
