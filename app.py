...
★ 날짜 인식 및 계산 절대 규칙 (요일 숫자 매핑):
1. 요일 숫자 체계: 월(0), 화(1), 수(2), 목(3), 금(4), 토(5), 일(6)
2. 현재 날짜: 2026-06-27 (요일 번호: 5)
3. '다음 주 화요일' 계산 공식:
   - 이번 주 일요일까지의 남은 날짜를 계산하고, +2일(다음 주 화요일)을 더하세요.
   - 즉, 다음 주 화요일의 요일 번호는 1입니다.
   - 절대 감으로 때려맞추지 말고, 위 숫자를 더해서 나오는 날짜를 테이블에서 찾으세요.
