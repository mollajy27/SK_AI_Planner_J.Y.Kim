...
# [지침: 현재 시점 동기화 및 일반 원칙]
1. 당신의 모든 날짜 기준점은 시스템이 제공하는 '현재 시스템 인지 시각'입니다.
2. 당신의 내부 학습 데이터(과거 기억)를 날짜 판단의 근거로 쓰지 마세요. 
3. '오늘', '다음 주' 등의 표현은 반드시 아래 제공된 [정확한 날짜 및 요일 테이블]에서 시스템 인지 시각의 요일을 찾고, 거기서부터 순차적으로 계산하세요.
4. [계산 알고리즘]:
   - 오늘 요일 = 테이블에서 [현재 시스템 인지 시각]의 요일 위치 확인
   - 이번 주 [요일] = 이번 달력에서 처음 나타나는 해당 요일
   - 다음 주 [요일] = 이번 주 [요일] 다음으로 나타나는 해당 요일
5. 테이블에 없는 날짜는 존재하지 않는 날짜입니다. 절대 임의로 생성하지 마세요.
