...
# 2. 실시간 현재 날짜 및 요일 맥락 계산
now = datetime.now()
current_time_str = now.strftime("%Y-%m-%d %H:%M")
weekday_list = ["월요일", "화요일", "수요일", "목요일", "금요일", "토요일", "일요일"]
current_weekday = weekday_list[now.weekday()]

# AI 에이전트에게 주입할 고도화된 시스템 프롬프트 (달력 연산 가이드 포함)
...
- 사용자가 '오늘', '내일', '다음 주 수요일', '7시' 등 상대적인 시간 표현을 쓰면, 반드시 이 기준 시각[{current_time_str}]과 현재 요일[{current_weekday}]을 바탕으로 정확한 연-월-일 및 24시간 형식(오후 7시면 19:00)을 계산하여 반영해야 합니다. 절대로 날짜와 요일을 매칭할 때 환각(오류)을 일으키지 마세요.
...
[
  ...
  {{"카테고리": "유연한 작업", "일시": "2026-07-01 15:00", "내용": "헬스장 운동", "비고": "가볍게 등 운동 1시간"}}
]
[/JSON_DATA]"""

# 3. 데이터 저장을 위한 Session State 초기화 (유저/어시스턴트 대화만 순수하게 유지)
...
with tab1:
    ...
    if user_input:
        ...
        # API를 호출할 때, 실시간으로 갱신된 system_instruction을 맨 앞에 조립해서 전송
        ...
        st.rerun() # 화면을 즉시 새로고침하여 최신 내용 반영
...
with tab2:
    ...
    if st.session_state.confirmed_events:
        ...
        # '일시' 컬럼을 기준으로 표를 시간 순서대로 정렬 (정렬 버그 수정)
        ...
        for date in unique_dates:
            with st.expander(f"📅 {date} 일자 계획 확인하기"):
                ...
                day_df = day_df.sort_values(by="일시", ascending=True) # 날짜 내에서도 시간순 정렬
    ...
    # 3. 시스템 전체 초기화 버튼 (대화 내역과 일정표 표 데이터를 동시에 포맷)
    st.markdown("---")
