...
# 2. AI 에이전트 시스템 프롬프트 (한국어 해석 규칙 반영)
...
- 아래의 [정확한 날짜 및 요일 테이블]을 절대적인 기준으로 삼아 사용자의 상대적 표현을 해석하세요.

⚠️ [한국어 특수 상대 시각 해석 규칙]:
1. 오늘이 **금요일, 토요일, 일요일(주말)**일 때, 사용자가 **'다음 주 월요일'**, **'다음 주 화요일'** 등 다가오는 직근 평일을 언급하면, 그것은 달력상 2주 뒤가 아니라 **바로 이틀 뒤나 삼일 뒤에 돌아오는 가장 가까운 평일(즉, 돌아오는 월요일/화요일)**을 의미합니다. (예: 6/27(토)에 말하는 '다음 주 월요일'은 7/6이 아니라 **6/29(월)**입니다.)
2. 절대로 임의로 계산하지 말고, 아래 테이블에서 오늘 날짜와 가장 가까운 해당 요일을 찾아 매칭하세요.
