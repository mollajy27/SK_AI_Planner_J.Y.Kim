...
system_instruction = r"""당신은 기존 플래너의 한계를 극복하는 '유연한 일정 관리 에이전트'입니다.
사용자와 대화하며 일정을 조율해 주세요.

# [핵심 날짜 계산 프로토콜 - 절대 준수]
1. 모든 날짜 계산은 "머릿속으로 연산"하지 말고, 하단의 [정확한 날짜 및 요일 테이블]을 "눈으로 스캔(Look-up)"하여 값을 찾으세요.
2. 사용자가 '다음 주 목요일'이라고 말하면:
   - STEP 1: 테이블에서 오늘(6/29 월요일)의 위치를 찾으세요.
   - STEP 2: 테이블에서 '목요일' 키워드를 찾으세요.
   - STEP 3: 오늘(6/29) 이후에 나타나는 두 번째 '목요일'이 '다음 주 목요일'입니다. (첫 번째는 이번 주 목요일)
   - STEP 4: 해당 행에 적힌 [YYYY-MM-DD] 값을 그대로 인용하여 답변하세요.
3. [금지]: '오늘 + 7일' 같은 수식은 절대 사용하지 마세요. 오직 테이블 기반의 [Look-up] 결과만 정답으로 간주합니다. 
4. 만약 테이블상의 날짜가 헷갈린다면, "테이블상으로 다음 주 목요일은 7월 9일입니다"라고 근거를 함께 제시하세요.
...
3. 일정의 이름을 임의로 추정하지 말고, 사용자가 (예:사용자가 '세미나 가기 전에 항상 1시간 30분 전부터 준비하고 이동하니까, 그 시간도 같이 맞춰서 수정해 달라'고 한다면, '준비 및 이동'을 일정 이름으로 추가)
...
★ [JSON 데이터 출력 규칙 - 매우 중요]:
1. [강제] 만약 사용자가 일정을 추가/수정하는 요청을 했다면, 설명보다 먼저 JSON 데이터를 생성하세요.
2. [강제] 만약 JSON 블록(`[JSON_DATA]...[/JSON_DATA]`)을 생성하지 않으면, 사용자의 일정 확정 요청은 실패로 간주합니다.    
3. 당신이 일정을 추가할 때, **요청받은 모든 일정을 하나의 JSON 배열 리스트로 묶어서 출력**하세요.
4. 여러 번 출력하지 말고, 한 번의 [JSON_DATA] 블록 안에 모든 일정을 담아 한 번에 출력하세요.
5. 모든 일정은 아래와 같은 JSON 리스트 형식으로 출력하세요. 예시:
[JSON_DATA]
[
  {"action": "create", "event": {"시작시간": "2026-07-09 11:30", "종료시간": "2026-07-09 13:00", "내용": "팀 점심", "카테고리": "카테고리 없음"}}
]
[/JSON_DATA]
"""
        ...
        with st.spinner("AI 비서가 일정을 연산하고 있습니다..."):
            try:
                ...
                if "[JSON_DATA]" in raw_response:
                    try:
                        # JSON 블록만 추출
                        content = raw_response.split("[JSON_DATA]")[1].split("[/JSON_DATA]")[0].strip()
                        # 불필요한 마크다운 문법 제거
                        ...
                        # 2. 리스트가 아니라면 리스트로 강제 변환
                        if isinstance(actions, dict):
                            actions = [actions]
                            
                        # 3. 각 action별로 안전하게 처리
                        for action_data in actions:
                            if action_data.get("action") == "create":
                                new_ev = action_data.get("event")
                                if new_ev:
                                    # 중복 확인
                                    is_duplicate = any(ev.get("시작시간") == new_ev.get("시작시간") for ev in st.session_state.confirmed_events)
                                    if not is_duplicate:
                                        new_ev["카테고리"] = new_ev.get("카테고리", "카테고리 없음")
                                        st.session_state.confirmed_events.append(new_ev)
                            elif action_data.get("action") == "delete":
                                target = action_data.get("target_keyword")
                                if target:
                                    st.session_state.confirmed_events = [ev for ev in st.session_state.confirmed_events if target not in ev.get("내용", "")]
                                    
                    except json.JSONDecodeError as e:
                        st.error(f"JSON 형식 오류: AI가 잘못된 데이터를 보냈습니다. raw: {content}")
                # ---------------------------------------------
                        pass
